...
def train(c):
    ...
    for step in range(1000):
        for agent in range(num_agents):
            ...
            # take action with max estimated value, but explore some too
            func = [ Q[agent][i] + (c * sqrt(log(step + 1) / (N[agent][i] + 1))) for i in range(k) ]
        ...
        plot(step, successes.mean(), '% Optimal Action - UCB', 'c: ' + str(c), new)
